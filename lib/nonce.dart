...
Uint8List randomBytes(int size) {
  ...
  return Uint8List.fromList(List.generate(size, (index) => random.nextInt(256)));
}
...
String generateNonce(PublicKey publicKey, int maxEpoch, dynamic randomness) {
  ...
  if (randomness is String) {
    ...
    bigNum = poseidonHash([ephPublicKey0, ephPublicKey1, BigInt.from(maxEpoch), randomness]);
  } else {
    throw ArgumentError('Invalid type for randomness. It should be either BigInt or String.');
  }

  Uint8List z = toBigEndianBytes(bigNum, 20);
  String nonce = base64UrlEncode(z).replaceAll('=', '').replaceAll('+', '-').replaceAll('/', '_');
  if (nonce.length != NONCE_LENGTH) {
    throw Exception('Length of nonce $nonce (${nonce.length}) is not equal to $NONCE_LENGTH');
  }
  return nonce;
}
